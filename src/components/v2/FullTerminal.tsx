...
export const FullTerminal: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            // grid dimensions change, but we'll just let pipes out-of-bounds trigger reset
        };
        window.addEventListener('resize', handleResize);
    ...
    const processCommand = async (cmdInput: string) => {
        ...
        switch (cmd) {
            ...
            case 'stats':
                ...
                setLines(prev => [...prev, ...stats].slice(-400));
                return;
            case 'logs':
            ...
            case 'neofetch':
                ...
                setLines(prev => [...prev, ...LinuxCommands.handleNeofetch(config)].slice(-400));
                return;
            ...
            case 'sudo':
                ...
                if (sudoRes.startDestruction) {
                    ...
                    const interval = setInterval(() => {
                        ...
                        // Chaotic glitching
                        if (progress % 10 === 0) {
                            setLines(prev => [...prev, { text: `[CRITICAL] Data corruption at 0x${Math.random().toString(16).slice(2, 6).toUpperCase()}`, type: 'error' as const }].slice(-400));
                        }
        ...
        if (outputLines.length > 0) {
            setLines(prev => {
                ...
                return newLines.slice(-400); // Keep only last 400 lines for performance
            });
        }
    };
    ...
    const handleCommand = async (e: React.FormEvent) => {
        ...
        if (val) {
            ...
            setLines(prev => prev.slice(-400));
        } else {
            const promptStart = isRoot ? config.terminal.promptRoot + ' ' : config.terminal.prompt + ' ';
            setLines(prev => [...prev, { text: promptStart, type: 'input' as const }].slice(-400));
            setInput("");
            setCursorPos(0);
        }
    };
 ...
 /_-'             '-\\`}
    ...
    return (
        ...
        >
            ...
            {isPipesActive && <canvas ref={pipesRef} className="absolute inset-0 opacity-60 pointer-events-none" />}
            ...
            {/* Main Terminal Area */}
            <div
                className={`flex-1 flex flex-col p-4 sm:p-8 bg-black/40 relative z-10 transition-transform force-select ${destructionProgress > 50 ? 'skew-x-2' : ''}`}
            ...
            >
                <div
                    ref={scrollRef}
                    aria-live="polite"
                    className="flex-1 overflow-y-auto space-y-1 scrollbar-hide pb-20 select-text force-select"
                >
                    {lines.map((line, i) => (
                        <TerminalRow
                            key={`line-${i}`}
